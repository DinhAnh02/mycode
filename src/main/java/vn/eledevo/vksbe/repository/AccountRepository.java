...
public interface AccountRepository extends BaseRepository<Accounts, Long>, JpaSpecificationExecutor<Accounts> {
    ...
    @Query(
            "SELECT new vn.eledevo.vksbe.dto.model.account.UserInfo(a.id,a.username,p.avatar, p.fullName, p.gender, p.phoneNumber,r.code, d.code,'1', a.isConditionLogin1, a.isConditionLogin2) "
                    + "FROM Accounts a "
                    + "JOIN a.roles r "
    ...
    @Query("SELECT new vn.eledevo.vksbe.dto.response.account.AccountSwapResponse(a.id, p.fullName, a.username) "
            + "FROM Accounts a "
            + "JOIN a.profile p "
